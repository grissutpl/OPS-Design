...
from PyQt5.QtWidgets import QMainWindow, QApplication, QComboBox, QTableWidgetItem, QMessageBox, QWidget
...
from src_calc.packdetails.details import detallado, detallado_a, detallado_b

from src_calc.motor.optimal import optimalframe
from src_calc.motor.inputs_refresh import ingr_actual_a, ingr_actual_b
...
from Gui.subDinamic_DataInput import Ui_MainWindow
# C L A S E    D E    L A    S U B V E N T A N A    D E    D A T O S    D E    E N T R A D A 
# ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯

class DataInput(QMainWindow, Ui_MainWindow):
...
#        loadUi('Gui/subDinamic_DataInput.ui', self)
        ...
        self.parent.actionLoad.setEnabled(True)
        self.parent.actionSave.setEnabled(True)

        self.parent.actionLoad.triggered.connect(self.load)    
        self.parent.actionSave.triggered.connect(self.save)    
...
#        self.pushButtonOptimizacion.clicked.connect(self.optimization)  
    ...
    def funcion_tread(self):
        ...
        msg.setText('''<b>This process can take a few minutes...</b>
...
#            self.tableWidget_DatosEntrada.setItem(inx, 24, i24)
            ...
            combobox1 = QComboBox()
            if str(i[1]) == 'Column':
                combobox1.addItems([str(i[1]), 'Beam'])
            elif str(i[1]) == 'Beam':
                combobox1.addItems([str(i[1]), 'Column'])
...
#        self.checkBox_DC.setChecked(True)
        ...
        msg1 = QMessageBox()
        msg1.setWindowIcon(QtGui.QIcon("ops_icon.jpg"))
        msg1.setWindowTitle('Advise')
        msg1.setText('''<b>The example has been loaded successfully.</b>
                    <br/><br/> Press Ok to continue.''')
    ...
    def mas(self):
        ...
        for row in range(self.tableWidget_DatosEntrada.rowCount() - 1, self.tableWidget_DatosEntrada.rowCount(), 1):
            for col in range(self.tableWidget_DatosEntrada.columnCount()):
                ...
                self.combobox1.addItems(['Column', 'Beam'])
...
#                self.tableWidget_DatosEntrada.setItem(row, 24, z24)
    ...
    def info(self):
        ...
        msgi.setWindowTitle("Software Limitations Advise")
        msgi.setWindowIcon(QtGui.QIcon("ops_icon.jpg"))
        msgi.setIcon(QMessageBox.Information)
        msgi.setText("<b>Recomendated information.</b>")
        
        msgi.setText('''<p>Its recomendate that your stucture has no more than 6 floors, we are in OMF systems. Beyond that the optimization process may take longer.</p>
                     <br/><br/>To understand better, visit the <a href='https://ingenieriasismica.utpl.edu.ec/?q=es/OPS_Design'>GRISS UTPL.</a> page, where you can find the manual and the respective tutorials.</a>''')
        msgi.exec_()   
    ...
    def dataframe_gen(self, table):
        try:
            ...
            QMessageBox.warning(self, 'Advise', 'You have to insert numeric values, the degrees of freedom must always be integer values. <p>Please correct, more info in GRISS UTPL.</p>')


        datos_frame = pd.DataFrame(datos_locales,columns=('Element', 'Type', 'Floor','X(i)', 'Y(i)', '⟲(i)','X(j)', 'Y(j)', '⟲(j)','x(i)','y(i)', 'x(j)', 'y(j)','14', '15', '16',
    ...
    #        arrows = ((self.ingreso_datos[i][11] - self.ingreso_datos[i][9])/space)+1    
            ...
            self.slope = 0
            if self.ingreso_datos[i][1] == 'Beam':
...
#            plt.xlabel(r"$\bf{" + 'Longitud\,(m)' + "}$\n\n${Elaborado\,\,por:\,Daniel\,\,Villarreal\,\,Leiva}$")
            ...
            plt.xlabel('Length (m)', fontweight='bold')
            plt.axis('equal')
        
            plt.plot((self.xi, self.xf), (self.yi, self.yf),  color='black' if self.ingreso_datos[i][1] == "Column" else "gray", linewidth=4)             


            self.anotations = plt.annotate(f"Element: {self.ingreso_datos[i][1]} {self.ingreso_datos[i][0]}\n{self.ingreso_datos[i][13]} - {self.ingreso_datos[i][14]}",
                         ...
                         xytext= (0, -15) if self.ingreso_datos[i][1] == "Beam" else (5, 0), #--> Ajusta la ubicación del texto
                         ha='center' if self.ingreso_datos[i][1] == "Beam" else "left", 
                         va="center")
#
#
...
#                         va="center")
            ...
            if self.slope == 0:
                ...
                #Cargas Distribuidas
                if self.ingreso_datos[i][1] == 'Beam' and abs(self.ingreso_datos[i][15]) > 0:
                ...
                #Cargas Laterales
                if self.ingreso_datos[i][1] == 'Beam' and abs(self.ingreso_datos[i][21]) > 0:
                ...
                # Puntuales    
                if self.ingreso_datos[i][1] == 'Beam' and abs(self.ingreso_datos[i][16]) > 0:
    ...
    def analisisdisenio(self):
        ...
        try:
            ...
            my_console = Logger('src_calc/data.txt') 
    ...
    #                frame(self.ingreso_datos)
                    ...
                    msg = QMessageBox()
                    msg.setWindowTitle('Message advise')
                    msg.setWindowIcon(QtGui.QIcon("ops_icon.jpg"))
                    msg.setText('''<b>The process has been executed successfully, continue with review results...</b>
                                <br/><br/> Press Ok to continue.''')
                    msg.setIcon(QMessageBox.Information)
                    msg.exec_()
                    ...
                    if 'Singular matrix' in str(err):
                        ...
                        QMessageBox.warning(self, 'Advise', 'The data are incorrect, please correct.')            
...
#        self.is_sending_data = True
        ...
        if self.D_criterio == 0:
            ...
            if self.lista_los_mejores == 'NONE':

#                self.is_sending_data = False # Aquí finalizo el hilo de ejecución -----
                self.parent.actionResultados.setEnabled(False)
                
                msg = QMessageBox()
                msg.setWindowTitle('Message advise')
                msg.setWindowIcon(QtGui.QIcon("ops_icon.jpg"))
                msg.setText('''<b>There are no an optimal option, the program could end!.</b>
                            <br/><br/> Press Ok to continue.''')
                msg.setIcon(QMessageBox.Information)
                msg.exec_()
...
#                self.is_sending_data = False # Aquí finalizo el hilo de ejecución -----
    ...
    def load(self, event):
        ...
        if (self.parent.ventanas[0] == 1 or self.parent.ventanas[0] == 2) and self.parent.ventanas[1] == 0:

            reply = QMessageBox.question(self, 'Advise', 'To load a new model, make sure that the window is empty and that you have saved the progress of your work if it exists. <p><b>¿Do you want to continue?</b></p>',
                ...
                try:
                    ...
                    if self.filename[0]:
                        ...
                        with open(self.path, "r") as f:
                            ...
                            print(lines)
                            print('')
                        self.general_data = ast.literal_eval(lines[0])
                        print(self.general_data)
                        print('')
        
        
        
                        for j in self.general_data[0]:
                
                            inxe = self.general_data[0].index(j)
                            self.tableWidget_DatosEntrada.insertRow(inxe)
                
                            i0 = QTableWidgetItem(str(self.tableWidget_DatosEntrada.rowCount()))
                            i0.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                
                            i2 = QTableWidgetItem(str(j[2]))
                            i2.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i3 = QTableWidgetItem(str(j[3]))
                            i3.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i4 = QTableWidgetItem(str(j[4]))
                            i4.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i5 = QTableWidgetItem(str(j[5]))
                            i5.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i6 = QTableWidgetItem(str(j[6]))
                            i6.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i7 = QTableWidgetItem(str(j[7]))
                            i7.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i8 = QTableWidgetItem(str(j[8]))
                            i8.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i9 = QTableWidgetItem(str(j[9]))
                            i9.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i10 = QTableWidgetItem(str(j[10]))
                            i10.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i11 = QTableWidgetItem(str(j[11]))
                            i11.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i12 = QTableWidgetItem(str(j[12]))
                            i12.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                #            i13 = QTableWidgetItem(str(j[13]))
                #            i13.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            
                            i15 = QTableWidgetItem(str(j[15]))
                            i15.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i16 = QTableWidgetItem(str(j[16]))
                            i16.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i17 = QTableWidgetItem(str(j[17]))
                            i17.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i18 = QTableWidgetItem(str(j[18]))
                            i18.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i19 = QTableWidgetItem(str(j[19]))
                            i19.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i20 = QTableWidgetItem(str(j[20]))
                            i20.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i21 = QTableWidgetItem(str(j[21]))
                            i21.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                            i22 = QTableWidgetItem(str(j[22]))
                            i22.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                #            i23 = QTableWidgetItem(str(j[23]))
                #            i23.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                #            i24 = QTableWidgetItem(str(j[24]))
                #            i24.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                
                            self.tableWidget_DatosEntrada.setItem(inxe, 0, i0)            
                            
                            self.tableWidget_DatosEntrada.setItem(inxe, 2, i2)
                            self.tableWidget_DatosEntrada.setItem(inxe, 3, i3)
                            self.tableWidget_DatosEntrada.setItem(inxe, 4, i4)
                            self.tableWidget_DatosEntrada.setItem(inxe, 5, i5)
                            self.tableWidget_DatosEntrada.setItem(inxe, 6, i6)
                            self.tableWidget_DatosEntrada.setItem(inxe, 7, i7)
                            self.tableWidget_DatosEntrada.setItem(inxe, 8, i8)
                            self.tableWidget_DatosEntrada.setItem(inxe, 9, i9)
                            self.tableWidget_DatosEntrada.setItem(inxe, 10, i10)
                            self.tableWidget_DatosEntrada.setItem(inxe, 11, i11)
                            self.tableWidget_DatosEntrada.setItem(inxe, 12, i12)
                #            self.tableWidget_DatosEntrada.setItem(inxe, 13, i13)
                
                
                
                            self.tableWidget_DatosEntrada.setItem(inxe, 15, i15)
                            self.tableWidget_DatosEntrada.setItem(inxe, 16, i16)
                            self.tableWidget_DatosEntrada.setItem(inxe, 17, i17)
                            self.tableWidget_DatosEntrada.setItem(inxe, 18, i18)
                            self.tableWidget_DatosEntrada.setItem(inxe, 19, i19)
                            self.tableWidget_DatosEntrada.setItem(inxe, 20, i20)
                            self.tableWidget_DatosEntrada.setItem(inxe, 21, i21)
                            self.tableWidget_DatosEntrada.setItem(inxe, 22, i22)
                #            self.tableWidget_DatosEntrada.setItem(inxe, 23, i23)
                #            self.tableWidget_DatosEntrada.setItem(inxe, 24, i24)
                            ...
                            combobox1 = QComboBox()
                            if str(j[1]) == 'Column':
                                combobox1.addItems([str(j[1]), 'Beam'])
                            elif str(j[1]) == 'Beam':
                                combobox1.addItems([str(j[1]), 'Column'])
                
                
                            combobox2 = QComboBox()
#                            if (str(j[14]) == '16X26' or str(j[14]) == '14X53'):
                            combobox2.addItems([str(j[14]), '4X13',	  '5X16',	  '5X19',	'6X8.5',	'6X9', 	'6X12',	'6X16',	'6X15',	'6X20',	'6X25',	
                                                   '8X10',	  '8X13',	  '8X15',	'8X18',	'8X21',	'8X24',	'8X28',	'8X31',	'8X35',	'8X40',	
                                                   '8X48',	  '8X58',	  '8X67',	'10X12',	'10X15',	'10X17',	'10X19',	'10X22',	'10X26',	'10X30',	
                                                  '10X33',	 '10X39',	 '10X45',	'10X49',	'10X54',	'10X60',	'10X68',	'10X77',	'10X88',	'10X100',	
                                                 '10X112',	 '12X14',	 '12X16',	'12X19',	'12X22',	'12X26',	'12X30',	'12X35',	'12X40',	'12X45',	
                            ...
                            self.tableWidget_DatosEntrada.setCellWidget(inxe, 1, combobox1)
                            self.tableWidget_DatosEntrada.setCellWidget(inxe, 14, combobox2)
                            self.tableWidget_DatosEntrada.setCellWidget(inxe, 13, combobox0)
                
                
                        # 2.1 Aquí lleno los imputs de inicialización
                        self.lineEdit_NodosTotales.setText(str(self.general_data[1])) 
                        self.lineEdit_NodosBase.setText(str(self.general_data[2]))
                ...
                #        self.checkBox_DC.setChecked(True)
                        ...
                        msg1.setWindowIcon(QtGui.QIcon("ops_icon.jpg"))
                        msg1.setWindowTitle('Advise')
                        msg1.setText('''<b>The model has been loaded successfully.</b>
                                    <br/><br/> Press Ok to continue.''')
                        msg1.setIconPixmap(QPixmap("checks.png"))
                        msg1.exec_()        
                #        self.pushButtonGraficaInicio.setEnabled(True)
            ...
            msg1.setWindowIcon(QtGui.QIcon("ops_icon.jpg"))
            msg1.setWindowTitle('Advise')
            msg1.setText('''<b>To load, go to the data window.</b>
                        <br/><br/> Press Ok to continue.''')
            msg1.exec_()                 
    ...
    def save(self):
        ...
        #        print(self.parent.DatosPrincipales)
                
                self.ingreso_datos = ingreso_de_datos_excel(self.parent.DatosPrincipales)       # Luego se ingresarán los datos.
                
                print(self.ingreso_datos)
                print(int(self.lineEdit_NodosTotales.text()))
                print(int(self.lineEdit_NodosBase.text()))
                
                self.savefile = QtWidgets.QFileDialog.getSaveFileName(None,'SaveTextFile','/', "Text Files (*.txt)")
        
                # This will let you access the test in your QTextEdit
                self.Text = [self.ingreso_datos, int(self.lineEdit_NodosTotales.text()), int(self.lineEdit_NodosBase.text())]
                ...
                if self.savefile[0]: 
                    ...
                    with open(self.savefile[0], 'w') as file:
                        file.write(str(self.Text))        
                
            except (ValueError):
                QMessageBox.warning(self, 'Advise', 'There is no data to save or it is incorrect. Please try again')        
            ...
            msg1.setWindowIcon(QtGui.QIcon("ops_icon.jpg"))
            msg1.setWindowTitle('Advise')
            msg1.setText('''<b>To save, go to the data window.</b>
                        <br/><br/> Press Ok to continue.''')
            msg1.exec_()  
